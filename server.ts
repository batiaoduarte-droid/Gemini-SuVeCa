...
let adminAppInstance: ReturnType<typeof initializeAdminApp> | null = null;
function getAdminApp() {
  if (!adminAppInstance) {
    ...
    if (existing.length > 0) {
      ...
      try {
        adminAppInstance = initializeAdminApp({ credential: applicationDefault() });
      } catch {
        try {
...
const requireFirebaseUser: express.RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    const decoded = await getAdminAuth(admin).verifyIdToken(token, true);
